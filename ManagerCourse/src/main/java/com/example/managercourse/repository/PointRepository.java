...
public interface PointRepository extends JpaRepository<Point, Integer> {
    ...
    @Query("SELECT new com.example.managercourse.dto.response.PointBySubjectResponse(s.subjectName, p.point) " +
            "FROM Point p " +
            "JOIN p.subject s " +
    ...
    @Query(value = "SELECT u.full_name, COALESCE(p.point, 0) AS point, u.id AS idUser, p.id AS idPoint\n" +
            ...
            "WHERE c.class_name = ?2", nativeQuery = true)
    List<Object[]> selectPointTeacher(String subjectName, String className);

    @Query("SELECT new com.example.managercourse.dto.response.PointBySubjectResponse(u.fullName, s.subjectName, p.point) " +
            "FROM ClassDetail cd " +
            "JOIN cd.aClass cl " +
            "JOIN cd.user u " +
            "JOIN u.pointList p " +
